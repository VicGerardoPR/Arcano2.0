                ...
                <div className="glass rounded-[3rem] overflow-hidden border-white/5 shadow-2xl">
                    <div className="grid grid-cols-1 lg:grid-cols-2">
                        ...
                        <div className="p-12 lg:p-20 bg-primary flex flex-col justify-between text-primary-foreground">
                            <div>
                                <h2 className="text-4xl md:text-5xl font-black mb-8 leading-tight">{t.contact.heading}</h2>
                            ...
                            <form action="https://api.web3forms.com/submit" method="POST" className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold uppercase tracking-widest text-foreground/40 ml-1">{t.contact.labels.name}</label>
                                        <Input name="name" required placeholder="John Doe" className="bg-white/5 border-white/10 h-14 rounded-2xl focus:ring-primary" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold uppercase tracking-widest text-foreground/40 ml-1">{t.contact.labels.email}</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-bold uppercase tracking-widest text-foreground/40 ml-1">{t.contact.labels.scope}</label>
                                    <select name="project_scope" className="w-full bg-white/5 border border-white/10 h-14 rounded-2xl px-4 text-sm focus:outline-none focus:ring-2 focus:ring-primary appearance-none">
                                        {t.contact.scopes.map((scope: string) => (
                                            <option key={scope} className="bg-background">{scope}</option>
                                        ))}
                                    </select>
                                </div>

                                <div className="space-y-2">
                                    <label className="text-xs font-bold uppercase tracking-widest text-foreground/40 ml-1">{t.contact.labels.message}</label>
                                    <textarea
                                        name="message"
                                        required
                                        placeholder="..."
